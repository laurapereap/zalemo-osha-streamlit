...
# Carpeta de salidas (modelos, reportes rápidos, etc.)
...
sep("Carga robusta: múltiples estrategias")

def try_read_csv(path):
    attempts = []

    # A1) dtype=str + engine c (rápido). Suelen fallar los CSV "rotos".
    attempts.append(dict(
        desc="A1: engine='c', dtype=str",
        kwargs=dict(dtype=str, low_memory=False)
    ))

    # A2) engine='python' + dtype=str (más tolerante con comillas y saltos)
    attempts.append(dict(
        desc="A2: engine='python', dtype=str",
        kwargs=dict(dtype=str, engine="python")
    ))

    # A3) engine='python' + sep auto + on_bad_lines='warn' (acepta líneas raras, no las salta)
    #  Nota: on_bad_lines='warn' intentará parsear; si no puede, avisa y omite esas líneas.
    attempts.append(dict(
        desc="A3: engine='python', sep autodetect, on_bad_lines='warn'",
        kwargs=dict(dtype=str, engine="python", sep=None, on_bad_lines="warn")
    ))

    # A4) engine='python' + escapechar='\\' + doublequote=True (típico cuando hay comillas internas)
    attempts.append(dict(
...
# Guardamos una copia "tal cual" leída para trazabilidad
...
sep("Normalización de nombres de columnas")
...
# Función auxiliar para encontrar columnas por posibles alias
...
# Intentamos localizar columnas típicas
...
print("Columna título (EventTitle):", title_col)
print("Columna descripción:", desc_col)
...
print(f"Filtrado filas sin fecha válida: {n_before} -> {df.shape[0]}")

sep("Duplicados")
# Definimos una llave razonable para duplicados (ajústala si tienes un ID)
...
# Guardamos una versión "limpia mínima"
df_min = df.copy()
...
# Elegimos columna de texto principal (prefiere descripción; sino título)
text_col = desc_col or title_col
assert text_col is not None, "No encuentro columna de texto (descripción o título). Revisa alias."

def clean_text(s):
    if pd.isna(s): return ""
    s = str(s).lower()
    s = re.sub(r"\s+", " ", s)
    s = re.sub(r"[^a-z0-9 \-_/]", " ", s)  # mantenemos algunos caracteres útiles
    return s.strip()
...
plt.xlabel("Año"); plt.ylabel("Número de eventos")
...
def default_family_rules(s):
    ...
    Devuelve una 'familia' más general a partir del título del evento.
    """
    s = str(s).lower()
    rules = [
...
sep("Distribución de familias (simplificadas)")
...
# Errores más comunes: dónde se confunde y ejemplos
err_df = pd.DataFrame({
...
print("\nTop 10 confusiones (y_true → y_pred):")
print(conf_counts)

display(err_df.head(10))

# === Curvas de calibración multiclase (OVR) usando from_predictions ===
...
# 2) Calibración (Platt scaling) sobre el pipeline multiclase
...
# 4) Elegimos 3 clases visibles (ajústalas si quieres otras)
...
ax.set_title("Figura 3. Curvas de calibración (Linear SVM calibrado - OVR)")
...
print(f"Métricas comparadas en: {cmp_path}")

# Reporte de clasificación en TEST (SVM) para anexos
...
plt.title("Comparación F1-macro (TEST)")
plt.ylabel("F1-macro")
plt.tight_layout()
...
# CALIBRACIÓN DE PROBABILIDADES (SVM) COMPATIBLE
...
# Construimos el estimador base (LinearSVC) y elegimos el nombre de parámetro correcto
...
# Métricas con el modelo calibrado
...
def infer_ui(texto):
    if not texto or len(texto.strip()) < 5:
        return "Escribe una descripción más detallada.", []
    top3 = predict_topk(texto, k=3)
    # Devolvemos predicción principal y tabla de top3
    pred, prob = top3[0]
    rows = [{"Clase": c, "Probabilidad": f"{p:.3f}"} for c,p in top3]
    return f"Predicción: {pred} (p≈{prob:.3f})", rows

with gr.Blocks() as demo:
    gr.Markdown("## Predicción de familia de accidente (SVM calibrado)")
    inp = gr.Textbox(label="Descripción del incidente")
...
# 1) Guardar matriz de confusión (SVM calibrado)
...
plt.title("Matriz de confusión (TEST, normalizada) — SVM calibrado")
...
# 2) Guardar top términos por clase (usamos el SVM no calibrado para coeficientes)
...
top_terms = {}
for i, cls in enumerate(clf.classes_):
    # LinearSVC no expone coef_ con shape [n_classes, n_features] si one-vs-rest -> sí expone
    coef = clf.coef_[i]
...
print("Top términos por clase guardados en:", terms_path)

# UMBRALES POR CLASE Y REGLA ANTI-"OTHER"

import numpy as np

# 1) umbrales por clase (ajústalos con sentido común tras mirar errores)
CLASS_THRESHOLDS = {
    "transport": 0.35,
    "cut_laceration": 0.30,
    "fire_explosion": 0.30,
    ...
    "electrical": 0.25,
    "fall": 0.25,
    "other": 0.40,   # ← más exigente para no caer en "other" por defecto
}

# 2) función de predicción con top-k, umbrales y regla contra "other"
def predict_with_thresholds(text, model=pipe_svm_cal, k=3, thresholds=CLASS_THRESHOLDS):
    ...
    # regla anti-"other": si top1 es other y top2 está razonable, elegir top2
    if c1 == "other":
        c2, p2 = ranked[1]
        # si top2 supera su umbral y no está muy lejos de "other"
        if p2 >= thresholds.get(c2, 0.25) and (p1 - p2) <= 0.08:
            c1, p1 = c2, p2

    # umbral por clase: si top1 no llega a su umbral, degradar a siguiente válida
...
print("Predicción:", pred, f"(p≈{prob:.3f})")
print("Top3:", topk)
...
PPE_CONTROLS = {
    "fall": {
        "ppe": ["Casco de seguridad", "Arnés con línea de vida", "Calzado antideslizante"],
        "controles": ["Barandillas/redes", "Señalización y orden", "Inspección de escaleras/andamios"]
    },
    "caught_in": {
        "ppe": ["Guantes anticorte", "Ropa ajustada", "Protección ocular"],
        "controles": ["Resguardos de máquina", "Bloqueo/etiquetado (LOTO)", "Zonas de exclusión"]
    },
    "struck_by": {
        "ppe": ["Casco", "Chaleco alta visibilidad", "Protección ocular"],
        "controles": ["Rutas peatonales", "Spotters en maniobras", "Mantenimiento de vehículos"]
    },
    "chemical": {
        "ppe": ["Guantes químicos", "Gafas/screen facial", "Respirador según SDS"],
        "controles": ["Ventilación", "Duchas/lavado ocular", "Almacenaje y etiquetado (SDS)"]
    },
    "electrical": {
        "ppe": ["Guantes dieléctricos", "Calzado aislante", "Pantalla facial ARC"],
        "controles": ["LOTO eléctrico", "Detectores de tensión", "Trabajos en BT/AT con PTW"]
    },
    "overexertion": {
        "ppe": ["Faja (si política lo permite)", "Guantes de agarre"],
        "controles": ["Ayudas mecánicas", "Rotación de tareas", "Formación en levantamiento seguro"]
    },
    "fire_explosion": {
        "ppe": ["Ropa ignífuga", "Guantes resistentes al calor", "Gafas/pantalla facial"],
        "controles": ["Permiso de trabajo en caliente", "Extintores y cortafuegos", "Control de atmósferas explosivas"]
    },
    "transport": {
        "ppe": ["Chaleco alta visibilidad", "Calzado de seguridad"],
        "controles": ["Separación peatón‑vehículo", "Señalistas", "Límites de velocidad"]
    },
    "cut_laceration": {
        "ppe": ["Guantes anticorte", "Mangas de protección", "Gafas"],
        "controles": ["Cuchillas de seguridad", "Resguardos", "Buenas prácticas de uso"]
    },
    "other": {
        "ppe": ["Protección estándar según tarea"],
        "controles": ["Evaluación específica del riesgo"]
    },
}
...
def infer_ui_v2(texto):
    if not texto or len(texto.strip()) < 10:
        return "Introduce una descripción más detallada.", [], [], []
    ...
    return f"Predicción: {fam} (p≈{p:.3f})", rows, ppe, controls

with gr.Blocks() as demo2:
    gr.Markdown("## Predicción de familia de accidente (SVM calibrado + umbrales)")
    inp = gr.Textbox(label="Descripción del incidente")
...
demo2.launch(share=False)  # pon share=True si necesitas URL pública
...
!cp -v /content/salidas_osha/* salidas_osha/ 2>/dev/null || echo "⚠️ Revisa que /content/salidas_osha tenga archivos"
